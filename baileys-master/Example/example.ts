...
async function example() {
    ...
    console.log('All Contacts ' + JSON.stringify(Object.keys(conn.contacts)))
    ...
    setInterval(async () => {
       count ++;
       const response = await conn.sendMessage('5516991538018@s.whatsapp.net', '*Ao que parece, todos os esforços do Itamaraty esto se revertendo em favor do pas asitico-comunista, tendo em vista o recorde na balança comercial nos ltimos dezoito meses. Definitivamente, os chineses no poderiam ter encomendado melhor preposto. Afinal, a partir da radicalizaço brasileira contra a Venezuela ao endossar os embargos e um possvel ataque norteamericano, nosso vizinho foi empurrado à compra de equipamentos militares russos e forçado a realizar acordos com os chineses em escalas jamais vistas antes. Ao renegar o pragmatismo histrico desempenhado pelo Brasil em seu entorno, trouxe para a nossa vizinhança uma crescente presença de potecias estrangeiras.* http://jornalpurosangue.com/2020/09/06/ernesto-araujo-o-maior-comunista-que-voce-ja-viu/\n*Embora seu poder de aço de amplo espectro poltico seja monumental, parece que o “Deep State” foi pego de surpresa nas ultimas eleiçoes, tanto nas americanas quanto nas brasileiras. As ferramentas digitais se desenvolveram muito rapidamente o que levou a uma quebra de paradigma tardiamente detectada. O comportamento dos eleitores, fortemente influenciadas pelas redes sociais, estava sendo nitidamente manipulado por agentes até ento no mapeados.* http://jornalpurosangue.com/2020/08/18/quao-profundo-e-hoje-o-deep-state/ ' + count , MessageType.text)
    }, 3 * 1000 )
}
